 ...
 */
const ChatProtoLAlwaysListening = () => {
  ...
  const handleVoiceTranscript = useCallback((transcript: string) => {
    console.log('[ChatProtoL-AlwaysListening] Voice transcript:', transcript);
  ...
  const handleManualSend = useCallback(() => {
    if (isListening) {
      console.log('[ChatProtoL-AlwaysListening] Manual send triggered');
      stopListening(false);
    }
        ...
        <div className="absolute top-64 left-1/2 -translate-x-1/2 z-20">
          ...
          >
            ...
            <span className="text-2xl text-muted-foreground">
              {isConnecting ? 'Connecting...' : 'Chat Ready (Always Listening)'}
            </span>
          </motion.div>
        </div>
      )}
...
export default ChatProtoLAlwaysListening;
